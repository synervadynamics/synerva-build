...
const metrics = [
  { label: "Avg. engagement lift", value: "+38%", note: "After voice + UX revamps" },
  { label: "Automation hours saved", value: "250+", note: "Quarterly composite" },
  { label: "Approval cycle reduction", value: "-80%", note: "Voice QA + Lucentra" },
  { label: "Campaign velocity", value: "2.1×", note: "Launch cadence improvement" }
];
...
const snapshots = [
  ...
  },
  {
    ...
    detail: "Tone, conviction, authenticity markers surfaced for executive review.",
    tag: "Verisense"
  }
];
    ...
    <main className="bg-[var(--bg)] text-white">
      <section className="relative isolate overflow-hidden px-6 pt-28 sm:px-10 lg:px-16">
        ...
        <div className="relative mx-auto flex max-w-5xl flex-col gap-6 rounded-[3rem] border border-white/10 bg-black/52 p-10 backdrop-blur-3xl shadow-[0_64px_180px_-88px_rgba(0,0,0,0.82)]">
          ...
          <h1 className="text-4xl leading-tight sm:text-5xl">{proofCopy.title}</h1>
        ...
        <div className="relative mx-auto max-w-6xl space-y-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.35em] text-white/60">Live proof board</p>
              <h2 className="text-3xl sm:text-4xl text-white">Signals in motion</h2>
            </div>
            <Link
              href="/contact"
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {metrics.map(metric => (
              ...
              >
                <p className="text-[0.65rem] uppercase tracking-[0.32em] text-white/60">{metric.label}</p>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {snapshots.map(card => (
              ...
              >
                <p className="text-[0.7rem] uppercase tracking-[0.3em] text-white/60">{card.tag}</p>
          ...
          <div className="rounded-[2rem] border border-white/12 bg-white/5 p-6 text-sm text-white/75">
            <p className="text-xs uppercase tracking-[0.35em] text-white/60">Measurement philosophy</p>
            <p className="mt-2">
              Every metric here ties to a real intervention. No vanity stats—only evidence of momentum gained. Detailed dashboards and audit trails are available on request.
            </p>
          </div>
        </div>
      </section>
