...
const preview = {
  subject: "Your Rockstar Server Playbook preview",
  path: "/downloads/rockstar-preview.pdf",
};
...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    console.info("[labs-interest] preview request (stub)", {
      email,
      previewLink,
    });
    return NextResponse.json({
      delivered: false,
      previewLink,
      note: "Configure RESEND_API_KEY to send automatically.",
    });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { delivered: false, message: "Failed to send preview" },
      { status: 500 },
    );
  }
}
