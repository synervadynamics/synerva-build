...
export default function SurfaceTensionGallery() {
  ...
  const showNext = useCallback(() => {
    setActiveIndex((idx) => {
      ...
      return (idx + 1) % surfaceTensionArtworks.length;
    });
  }, []);
  ...
  const showPrev = useCallback(() => {
    setActiveIndex((idx) => {
      if (idx === null) return idx;
      return (idx - 1 + surfaceTensionArtworks.length) % surfaceTensionArtworks.length;
    });
  }, []);
    ...
    <section className="quiet-divine-gallery space-y-6">
      ...
      {activeArtwork ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="surface-tension-modal-title"
        >
          <button
            type="button"
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                >
                  <p className="text-xs uppercase tracking-[0.35em] text-white/60">
                    {activeArtwork.artworkNumber}
                  </p>
                  <h3
                    id="surface-tension-modal-title"
                    className="mt-3 text-2xl font-semibold"
                  >
                    {activeArtwork.title ?? EMPTY_PLACEHOLDER}
                  </h3>
                </div>
