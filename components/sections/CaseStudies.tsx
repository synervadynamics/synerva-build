...
function normalizeItems(items: unknown): Item[] {
  ...
  const cleaned = items.filter(Boolean).map((raw, idx) => {
    const obj =
    ...
    const ctaLabel =
      typeof obj.ctaLabel === "string" && obj.ctaLabel.trim().length > 0
        ...
        : undefined;
    const ctaHref =
      typeof obj.ctaHref === "string" && obj.ctaHref.trim().length > 0
        ? obj.ctaHref.trim()
        : "#";
    const image: Img | undefined =
      typeof obj.image === "string" || typeof obj.image === "object"
        ? (obj.image as Img)
        : undefined;
    const mediaLabel =
      typeof obj.mediaLabel === "string" && obj.mediaLabel.trim().length > 0
        ? obj.mediaLabel
        : undefined;
    ...
    <section id="cases" className="py-24 sm:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <header className="mb-12 max-w-3xl">
          ...
          {intro ? (
            <p className="mt-3 text-base/7 text-neutral-400">{intro}</p>
          ) : null}
        </header>
            ...
            return (
              ...
              >
                <div className="space-y-4">
                  ...
                  {body ? (
                    <p className="text-sm text-neutral-300 leading-relaxed sm:text-base">
                      {body}
                    </p>
                  ) : null}
                  {ctaLabel ? (
                ...
                <div>{renderMedia(img, mediaLabel)}</div>
              </article>
            );
          })}
        </div>
